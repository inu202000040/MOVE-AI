...
완료 기록에는 판정에 사용한 승인 프레임/PNG 식별자, 데이터 팩 버전, 네 화면 크기 결과, 수치 골든 결과, 상태 시나리오 결과, 접근성 결과, 남은 P0/P1만 적는다. 구현 위치나 과거 구현 이력은 완료 증거로 사용하지 않는다.
